    ...
    if promo:
        if promo.status:
            await m.bot.edit_message_text(message_id=data['message_id'], chat_id=m.from_user.id,
                                          text="Такой промокод уже существует. Измените название промокода")
        else:
...
async def create_promo_uses(m: Message, state: FSMContext):
    data = await state.get_data()
    await m.bot.edit_message_text(message_id=data['message_id'], chat_id=m.from_user.id,
                                  text="Введите время, на которое промокод будет выдавать продукт\n\n1 час - 1h\n1 день - 1d\n1 месяц - 1m")
...
async def create_promo_duration(m: Message, state: FSMContext):
    data = await state.get_data()
    await m.bot.edit_message_text(message_id=data['message_id'], chat_id=m.from_user.id,
                                  text="Введите время действия промокода\n\n1 час - 1h\n1 день - 1d\n1 месяц - 1m")
...
async def create_promo_end_date(m: Message, state: FSMContext):
    ...
    await m.bot.edit_message_text(message_id=data['message_id'], chat_id=m.from_user.id,
                                  ...
                                  reply_markup=await ikb.create_promocode_product_selection_keyboard(data['promo_name'],
                                                                                                     data['promo_uses'],
                                                                                                     data[
                                                                                                         'promo_duration'],
                                                                                                     data[
